...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Nova Tag</h1>
    ...
    <form action="{{ route('admin.tags.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
        ...
        <div class="mb-4">
            ...
            <div class="flex items-center">
                <input type="color" id="color" name="color" class="w-16 h-10 border border-gray-300 rounded-lg cursor-pointer" value="{{ old('color', '#3B82F6') }}" required>
                <span class="ml-4 text-sm text-gray-600" id="colorCode">{{ old('color', '#3B82F6') }}</span>
            </div>
        </div>
