...
<div class="flex-1 flex flex-col overflow-hidden">
    ...
    <div class="h-16 px-6 sticky top-0 z-40 bg-surface/80 backdrop-blur-md border-b border-outline-variant flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-on-surface-variant mt-1">{{ $user->getRoleLabel() }} • {{ $user->getSectorName() }}</p>
        </div>
        ...
        <div class="max-w-4xl space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl border border-outline-variant shadow-sm p-6">
                        ...
                        <div class="space-y-3">
                            <div>
                                <p class="text-xs text-on-surface-variant font-semibold mb-1">Email</p>
                                <p class="text-sm text-on-surface">{{ $user->email }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-on-surface-variant font-semibold mb-1">Status Online</p>
                                <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium {{ $user->status === 'online' ? 'bg-secondary-container/20 text-secondary' : 'bg-outline-variant/20 text-on-surface-variant' }}">
                    ...
                    <div class="bg-white rounded-xl border border-outline-variant shadow-sm p-6">
                        ...
                        <div class="space-y-3">
                            <div>
                                <p class="text-xs text-on-surface-variant font-semibold mb-1">Cargo</p>
                                <p class="text-sm text-on-surface">{{ $user->getRoleLabel() }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-on-surface-variant font-semibold mb-1">Setor</p>
                                <p class="text-sm text-on-surface">{{ $user->getSectorName() }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-on-surface-variant font-semibold mb-1">Status</p>
                                <span class="px-2 py-1 rounded text-xs font-medium {{ $user->is_active ? 'bg-secondary-container/20 text-secondary' : 'bg-outline-variant/20 text-on-surface-variant' }}">
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl border border-outline-variant shadow-sm p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                <p class="text-xs text-on-surface-variant font-semibold mb-2">Conversas Ativas</p>
                                <div class="flex items-end gap-3">
                                    <div class="text-3xl font-bold text-secondary">{{ $user->agentCapacity->activeConversationsCount() }}</div>
                                    <div class="text-on-surface-variant text-sm">de {{ $user->agentCapacity->max_conversations }}</div>
                                </div>
                            ...
                            <div class="pt-2 border-t border-outline-variant">
                                <p class="text-xs text-on-surface-variant font-semibold mb-1">Máximo Permitido</p>
                            ...
                            <div class="p-3 bg-outline-variant/20 rounded-lg">
                                <p class="text-xs text-on-surface-variant">Não está recebendo novas conversas</p>
                            </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl border border-outline-variant shadow-sm p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-start justify-between p-3 rounded-lg bg-surface-container-low hover:bg-surface-container transition-colors">
                        <div class="flex-1">
                            <p class="font-medium text-on-surface">{{ $conversation->contact->name ?? 'Contato Desconhecido' }}</p>
                            <p class="text-xs text-on-surface-variant">{{ $conversation->contact->phone ?? '—' }}</p>
                            <p class="text-xs text-on-surface-variant mt-1">
                                @if($conversation->status === 'in_attendance')
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-secondary-container/20 text-secondary">
                                        <span class="w-1.5 h-1.5 rounded-full bg-secondary"></span>
                                        Em atendimento
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-outline-variant/20 text-on-surface-variant">
                ...
                <div class="text-center py-8">
                    <span class="material-symbols-outlined text-4xl text-on-surface-variant mb-2">chat_bubble_outline</span>
                    <p class="text-on-surface-variant text-sm">Nenhuma conversa ativa</p>
                </div>
                @endif
            </div>
