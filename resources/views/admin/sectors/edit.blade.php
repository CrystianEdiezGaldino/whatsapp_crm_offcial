...
<div class="flex-1 flex flex-col overflow-hidden">
    ...
    <div class="h-16 px-6 sticky top-0 z-40 bg-surface/80 backdrop-blur-md border-b border-gray-200 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-gray-600 mt-1">Atualize as informações de {{ $sector->name }}</p>
        </div>
    </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                ...
                <form action="{{ route('admin.sectors.update', $sector->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="p-4 bg-gray-100 rounded-lg border border-gray-200">
                        ...
                        <p class="text-xs text-gray-600">A opção do teclado não pode ser alterada. Se precisar mudar, delete e crie um novo setor.</p>
                    </div>

                    <!-- Name -->
                    <div>
                        <label class="text-sm font-semibold text-on-surface block mb-2">Nome do Setor</label>
                        <input type="text" name="name" value="{{ old('name', $sector->name) }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-secondary focus:border-secondary {{ $errors->has('name') ? 'border-error' : '' }}" placeholder="Ex: Financeiro, Secretaria, Vendas" required>
                    ...
                    <div>
                        ...
                        <textarea name="description" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-secondary focus:border-secondary {{ $errors->has('description') ? 'border-error' : '' }}" placeholder="Descrição do setor (opcional)" rows="2">{{ old('description', $sector->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-600 mt-1">Se deixar em branco, usará: &quot;Você foi redirecionado para {{ $sector->name }}. Um atendente irá responder em breve.&quot;</p>
                    </div>
                    ...
                    <div>
                        ...
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $sector->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-200">
