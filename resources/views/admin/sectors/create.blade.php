...
<div class="flex-1 flex flex-col overflow-hidden">
    ...
    <div class="h-16 px-6 sticky top-0 z-40 bg-surface/80 backdrop-blur-md border-b border-outline-variant flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-on-surface-variant mt-1">Crie um novo setor para o pré-atendimento</p>
        </div>
    </div>
            ...
            <div class="bg-white rounded-xl border border-outline-variant shadow-sm p-6">
                ...
                <form action="{{ route('admin.sectors.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="p-4 bg-surface-container rounded-lg border border-outline-variant">
                        ...
                        <p class="text-xs text-on-surface-variant">Esta opção será usada no pré-atendimento IVR. O cliente digita {{ $nextOption }} para acessar este setor.</p>
                    </div>

                    <!-- Name -->
                    <div>
                        <label class="text-sm font-semibold text-on-surface block mb-2">Nome do Setor</label>
                        <input type="text" name="name" value="{{ old('name') }}" class="w-full border border-outline-variant rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-secondary focus:border-secondary {{ $errors->has('name') ? 'border-error' : '' }}" placeholder="Ex: Financeiro, Secretaria, Vendas" required>
                    ...
                    <div>
                        ...
                        <textarea name="description" class="w-full border border-outline-variant rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-secondary focus:border-secondary {{ $errors->has('description') ? 'border-error' : '' }}" placeholder="Descrição do setor (opcional)" rows="2">{{ old('description') }}</textarea>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-on-surface-variant mt-1">Se deixar em branco, usará: &quot;Você foi redirecionado para [Nome do Setor]. Um atendente irá responder em breve.&quot;</p>
                    </div>
