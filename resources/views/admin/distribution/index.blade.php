        ...
        <div class="grid grid-cols-1 gap-6 max-w-7xl">
                ...
                <form action="{{ route('admin.distribution.settings') }}" method="POST" class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex gap-2">
                            <label class="flex items-center gap-2 p-3 border-2 rounded-lg cursor-pointer mode-label {{ $settings->isManual() ? 'border-secondary bg-secondary/10' : 'border-gray-200' }}" data-mode="manual">
                                ...
                                <span class="text-xs text-gray-600">(Agentes clamam conversas)</span>
                            </label>

                            <label class="flex items-center gap-2 p-3 border-2 rounded-lg cursor-pointer mode-label {{ $settings->isAutomatic() ? 'border-secondary bg-secondary/10' : 'border-gray-200' }}" data-mode="automatic" title="Novos leads são automaticamente atribuídos aos agentes em ordem round-robin">
                                ...
                                <span class="text-xs text-gray-600">(Sistema distribui em round-robin)</span>
                            </label>
                        </div>
                    </div>

                    <!-- Overflow Action (só mostrar se automático) -->
                    <div class="space-y-4 pt-4 border-t border-gray-200" id="overflowSection" style="{{ !$settings->isAutomatic() ? 'display: none;' : '' }}">
                        ...
                        <select name="overflow_action" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-secondary focus:border-secondary">
                            ...
                            <tr class="hover:bg-gray-100-low">
                                ...
                                <td class="px-4 py-3">
                                    <div class="w-24">
                                        ...
                                        <p class="text-xs text-gray-600 mt-1">{{ max(0, $maxCapacity - $activeCount) }} vagas</p>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                ...
                <div class="space-y-2 max-h-96 overflow-y-auto">
                    ...
                    <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-600 truncate">{{ $lead['last_message'] }}</p>
                        </div>
                        <div class="text-right ml-4 shrink-0">
                            <p class="text-xs font-semibold text-error">{{ $lead['wait_time'] }}min</p>
                            <p class="text-xs text-gray-600">aguardando</p>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                ...
                <div class="space-y-2 max-h-96 overflow-y-auto">
                    ...
                    <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-100-low">
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-on-surface">Conversation #{{ $assignment['conversation_id'] }}</p>
                            <p class="text-xs text-gray-600">{{ $assignment['agent_name'] }}</p>
                        </div>
                        <span class="text-xs text-gray-600 whitespace-nowrap ml-4">{{ $assignment['time_ago'] }}</span>
                    </div>
                    @endforeach
                </div>
                @else
            ...
            <div class="grid grid-cols-4 gap-4">
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-semibold text-gray-600 uppercase">Total de Agentes</p>
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-semibold text-gray-600 uppercase">Carga Total</p>
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-semibold text-gray-600 uppercase">Agentes Cheios</p>
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-semibold text-gray-600 uppercase">Na Fila</p>
    ...
    <div class="bg-white rounded-xl shadow-lg max-w-sm w-full p-6">
        ...
        <form id="capacityForm" class="space-y-4">
            ...
            <div>
                ...
                <input type="number" id="maxConversations" name="max_conversations" min="1" max="100" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-1 focus:ring-secondary focus:border-secondary">
            </div>

            <div class="flex justify-end gap-2">
                <button type="button" onclick="document.getElementById('capacityModal').classList.add('hidden')" class="px-4 py-2 border border-gray-200 rounded-lg text-on-surface hover:bg-gray-100 transition-colors">
                    Cancelar
                </button>
                <button type="submit" class="px-4 py-2 bg-secondary text-on-secondary rounded-lg font-semibold hover:bg-secondary/90 active:scale-95 transition-all">
                    Salvar
...
<script>
    ...
    document.querySelectorAll('.mode-radio').forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            this.closest('.mode-label').classList.remove('border-gray-200');
